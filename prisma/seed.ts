...
async function createTimestamps() {
	...
	for (const content of contentItems) {
		for (const employee of employeeData) {
			...
			const ONE_DAY = 24 * 60 * 60 * 1000
			// within the past 30 days, at least one day ago
			const recentlyViewed = new Date(Date.now() - ONE_DAY - Math.floor(Math.random() * 30 * ONE_DAY))
			const recentlyEdited = new Date(Date.now() - ONE_DAY - Math.floor(Math.random() * 30 * ONE_DAY))
