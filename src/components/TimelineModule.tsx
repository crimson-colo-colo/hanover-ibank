...
import { Group, Paper, Stack, Text, Timeline, Flex, Tooltip, ActionIcon, Combobox, InputBase, useCombobox, Image} from "@mantine/core"
import { IconSearch, IconLoader2, IconCircleX} from "@tabler/icons-react"
...
export function TimelineModule() {
    ...
    const { data : allUserActivity } = useQuery(trpc.activityLogging.listUserActivity.queryOptions({employeeId: employeeID}))
    const { data: userInfo } = useQuery(trpc.admin.listUsers.queryOptions())
    const contentIDs = allUserActivity?.map((item) => item.contentId ?? "") ?? []
    const { data: allContentTitles } = useQuery(trpc.content.getTitles.queryOptions({contentIds: contentIDs}))
    ...
    return (
        <Paper withBorder p="md" radius="md" h="100%">
            <Group gap={6} justify="space-between">
                ...
                {searching ? <ContentOwnerSearch onSubmit={setEmployeeID} setSearching={setSearching} inputRef={inputRef} /> :
                    <ActionIcon variant="subtle" mb="md" onClick={() => {
...
function ContentOwnerSearch({onSubmit, setSearching, inputRef} : {onSubmit: (val : string | undefined) => void, setSearching: (val : boolean) => void, inputRef: RefObject<HTMLInputElement | null>}) {
            ...
            >
                <Combobox.Target>
                    <InputBase
                        ref={inputRef}
                        rightSection={
                            searchResults.isFetching ? <IconLoader2 size={18} /> : <ActionIcon variant="transparent" onClick={(e) => {
                                e.stopPropagation()
                                setSelected(false)
