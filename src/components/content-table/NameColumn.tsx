...
export function NameColumn({info, openFilePreview, profile} : {info : CellContext<ContentListItem, string>, openFilePreview: (item: ContentListItem, type: FileType) => void, profile: profileType | undefined}) {
    ...
    if (item.type === "Link") {
            ...
            <div className="flex items-center gap-2 min-w-0">
                ...
                {info.row.original.checkedOutBy !== null &&
                    (info.row.original.checkedOutBy.id === profile?.id ? (
                        <Tooltip withArrow arrowSize={8}
                                 label="You have this link checked out">
                        ...
                        >
                            <IconPencilOff
                                className="hover:bg-red-500 hover:rounded-md shrink-0"
                                size={24}/>
                        </Tooltip>
                    ))}
            </div>
        ...
        return (
            <div className="flex items-center gap-2">
                <FileTypeIcon fileType={fileType ?? FileType.Unknown} size={22}
                              strokeWidth={1.5}/>
                <button
                ...
                {info.row.original.checkedOutBy !== null &&
                    (info.row.original.checkedOutBy.id === profile?.id ? (
                        <Tooltip withArrow arrowSize={8}
                                 label="You have this file checked out">
