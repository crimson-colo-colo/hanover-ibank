...
import {fuzzyFilter, fuzzySort, tagFilterFn, checkedOutByFilterFn, customFilterFunction} from "@/lib/table.ts"
    ...
    const columns = useMemo(
        () => [
                ...
                cell: (info) => (
                    ...
                    >
                        {favoriteContent.isPending || unfavoriteContent.isPending ? (
                            ...
                            <IconStarFilled
                                className="fill-[#f8de1f] content-favorite stroke-2 hover:stroke-black stroke-transparent"/>
                        ) : (
            ...
            columnHelper.accessor("expirationDate", {
                ...
                cell: (info) => (
                    <span title={info.getValue().toLocaleString()}
						...
						{formatDistanceToNow(info.getValue(), {addSuffix: true}).replace(/^(in )?about /, "$1")}
					</span>
                ),
            }),
            ...
            columnHelper.display({
                id: "actions",
                cell: (info) => (
                    <ActionColumn info={info} selectContentForCheckout={selectContentForCheckout}
                                  openCheckOutModal={openCheckOutModal}
                                  openCheckInModal={openCheckInModal} profile={profile}/>
                )
            }),
            columnHelper.accessor("checkedOutBy", {
                ...
                enableSorting: false,
                cell: (info) => (
                    info.row.original.checkedOutBy?.name
                ),
                                    ...
                                    <Flex gap="sm">
                                        ...
                                        {header.column.id !== "favorited" &&
                                            (header.column.getIsSorted() === "asc" ? (
                                                <IconSortAscending2 className="inline shrink-0"
                                                                    size={20}/>
                                            ) : header.column.getIsSorted() === "desc" ? (
                                                <IconSortDescending2 className="inline shrink-0"
                                                                     size={20}/>
                                            ) : null)}
                                    </Flex>
                                </Table.Th>
    ...
    function CheckInModal({
        ...
        return (
            <Modal opened={opened} onClose={onClose} title="Confirm check in">
                <Text>
                    Ready to check this content back in? Others will be able to edit it once it's
                    checked in.
                </Text>
